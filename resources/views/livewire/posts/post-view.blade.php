    ...
    <x-card class="post-card">
                ...
                <div class="mdc-layout-grid__cell--span-8">
                    ...
                    <div id="post-options-{{$this->post->id}}" class="post-options">
                        ...
                        <x-menu-surface class='share-menu' id="share-menu-{{$this->post->id}}" anchor-id="share-button-{{$post->id}}" fixed>
                            ...
                            <x-button id="copy-share-link-button-{{$this->post->id}}" iconButton icon="content-copy" :aria-label="__('Copy share link to clipboard')" tabindex="-1" />
                        </x-menu-surface>
        ...
        <section class="display-comments" id="display-comments-{{$this->post->id}}">
            ...
            @foreach($this->post->comments as $c)
                    <livewire:posts.comment-component :wire:key="$c->id . $c->content" :c="$c" :post="$this->post"
                            :tabindex="$this->post->comments->search($c)" ></livewire:posts.comment-component>
            @endforeach
            </x-list>
        </section>
    </x-card>
